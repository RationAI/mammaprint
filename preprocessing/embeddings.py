...
class Virchow2(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor: # TODO
...
def load_dataset(uris: Iterable[str]) -> TilesPredict:
    ...
    return TilesPredict(
        ...
        transforms=A.Compose(
            [
                A.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)), # TODO
            ]
        ),
    )
...
@hydra.main(
    config_path="../configs",
    config_name="preprocessing/embeddings",
    version_base=None,
)
@autolog
def main(config: DictConfig, logger: MLFlowLogger) -> None:
    ...
    with torch.no_grad():
        ...
        for slide_dataset in tqdm(dataset.generate_datasets()):
            slide_name = str(slide_dataset.slide_metadata["id"])
