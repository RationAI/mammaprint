...
def convert_tiff_to_tiff_parallel(input_file, output_file, tile_size=256, num_threads=4, batch_size=16):
    ...
    and saves the result as a tiled .tiff image. Processing is done in parallel using processes.
    ...
    logging.info(f"Converting {input_file} to {output_file} using batch processing and parallel processes")
    ...
    with ProcessPoolExecutor(max_workers=num_threads) as executor:
        futures = [executor.submit(process_tile, input_file, tile_size, x, y) for (x, y) in all_tiles]
