...
class VGG16Features:
    def __new__(cls, weights=None, model_uri=None, trainable=True) -> torch.nn.Module:
...
class VGG16CustomLayerCut:
    @classmethod
    def _get_leaf_layers(cls, m):
        children = list(m.children())
        if not children:
            return [m]
        leaves = []
        for leaf in children:
            leaves.extend(cls._get_leaf_layers(leaf))
        return leaves

    def __new__(
        cls, layer_idx=-1, weights=None, model_uri=None, trainable=True
    ) -> torch.nn.Module:
        if weights is not None and model_uri is not None:
            raise ValueError("Only one of weights or model_uri can be specified")
        if model_uri:
        ...
        if not trainable:
            for param in donor.parameters():
                param.requires_grad = False
        donor_layers = cls._get_leaf_layers(donor)
        print(f"Network disected on {donor_layers[layer_idx]}")
        return torch.nn.Sequential(*donor_layers[:layer_idx])
