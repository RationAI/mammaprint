...
# Local Imports
from histopipe.datamodule.datasources import BaseDataSource
...
class SequentialRandomTreeSampler(TreeSampler):
    """SequentialRandomTreeSampler.

    SequentialRandomSampler randomly chooses leaves, alternating nodes from
    different classes, and returns their data content or its subset of size
    epoch_size.

    Supports multi-level sampling by including 'index_level'.

    The algorithm starts at the left-most leaf and returns its content or its
    subset. The SequentialRandomTreeSampler moves to a random leaf
...
class RandomFilterTreeSampler(RandomTreeSampler):
    def __init__(
        ...
        super().__init__(index_levels, seed, epoch_size)
    ...
    def build_inner_structure(self, data_source: BaseDataSource) -> None:
        ...
        if self.target_label is None:
            dataset.class_id = False
        else:
            if self.strict:
                dataset.class_id = dataset.class_id.apply(
                    lambda classes: {self.target_label} == set(classes)
                )
            else:
                dataset.class_id = dataset.class_id.apply(
                    lambda classes: self.target_label in set(classes)
                )
...
class SequentialFilterTreeSampler(SequentialTreeSampler):
    def __init__(
        self,
        index_levels: list[str],
        advance_to_next: bool,
        ...
        super().__init__(index_levels, advance_to_next)
        self.target_label = target_label
        self.strict = strict
