...
**Repository:** `mammaprint` (RationAI)
**Context:** This is a research-focused machine learning repository dedicated to predicting cancer type and/or mammaprint index from breast WSIs in digital pathology. 
...
- **Experiment Tracking (MLflow):** When PRs add new loss functions, evaluation metrics, or training loops in `ml/`, ensure that these new metrics are properly logged to MLflow.
- **Repository Structure:**
  - `preprocessing/`: Ensure data transformations (tiling, QC, tissue masks) are logically sound.
  - `ml/`: Focus on training loops, PyTorch Lightning modules, and model definitions.
