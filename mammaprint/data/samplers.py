...
from mammaprint.data.datasources import BaseDataSource
    ...
    def get_sample(self) -> list[dict]:
        ...
        for _ in range(self.num_slides):
            ...
            if len(chosen_tiles) < self.tiles_per_bag:
                ...
                chosen_tiles = chosen_tiles.sample(
                    n=self.tiles_per_bag, replace=True
                ).to_dict("records")
            else:
                # Sample exactly tiles_per_bag tiles without replacement
                chosen_tiles = chosen_tiles.sample(
                    n=self.tiles_per_bag, replace=False
                ).to_dict("records")
        ...
        log.info(
            f"Sampled {len(samples)} slides with a total of {total_tiles} tiles successfully."
        )
    ...
    def get_sample(self) -> list[dict] | None:
        ...
        if res is not None:
            if isinstance(res, pandas.DataFrame):
                ...
                logging.warning(
                    f"Unexpected type for `res`: {type(res)}; expected a DataFrame."
                )
        else:
        ...
        logging.info(
            f"Sampled data contains {len(samples)} entries with a total of {total_tiles} tiles."
        )
