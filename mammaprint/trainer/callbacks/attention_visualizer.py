...
logger = logging.getLogger("callbacks/attention_visualizer")


class AttentionVisualizer(DataloaderAgnosticCallback):
    ...
    def on_test_dataloader_start(
        ...
        logger.debug("Creating new attention visualizer.")
    ...
    def on_test_dataloader_end(
        ...
        logger.debug("Saving attention heatmap.")
        ...
        logger.debug(f"attention heatmap saved to: {artifact_uri}")
        stripped_uri = artifact_uri.removeprefix("mlflow-artifacts:/")
        logger.debug(f"saving attention heatmap URI to the cache as {stripped_uri}")
    ...
    def on_test_batch_end(
        ...
        for i in range(batch_size):
            ...
            # Scale weights for better visualization, other normalizations like logaritmic can be used
