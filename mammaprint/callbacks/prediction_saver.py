...
import torch
from pyarrow.parquet import ParquetWriter, read_table, write_table
...
class ParquetPredictionSaver(DataloaderAgnosticCallback):
    ...
    def __init__(self, save_dir: str) -> None:
        ...
        self.writer2 = ParquetWriter(
            self.save_dir + "/slides_batch.parquet", schema_slides
        )
    ...
    def on_test_batch_end(
        ...
        model_output_processed = self._preprocess_data(
            outputs["outputs"], target_dtype=np.float32
        )
        ...
        if isinstance(mammaprint_value, torch.Tensor):
            mammaprint_value = (
                mammaprint_value.cpu().tolist()
                if mammaprint_value.dim() > 0
                else mammaprint_value.item()
            )
